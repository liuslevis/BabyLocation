...
@app.route('/baby/<name>/lat/<latitude>/long/<longitude>',methods=['GET'])
def updatetrack(name,latitude,longitude):
    ...
    # Write to Redis
    key = 'baby:'+name+':tracklist'
    value = lat_long_time
    r = getRedis()
    r.rpush(key,value)

    info = 'UPDATE: a baby  @(%s,%s,%s)'%(name,latitude,longitude) + ' Redis: RPUSH(%s,%s)'%(key,value)
    return info  

# Parent App Query a track of baby
# return JSON  {timeseq:[str,...],latseq:[str,...],longseq:[str,...]} 
@app.route('/baby/<name>/track',methods=['GET'])
def querytrack(name):
    # Read from Redis
    key = 'baby:'+name+':tracklist'
    r = getRedis()
    ...
    info =  'QUERY: track of @%s. %d record found.'%(name,len(timeseq))
    print info
    return json
...
def index():
    ...
    <body>
        <h1>Useage</h1>
            ...
            <h2>
                update a baby's location: GET http://localhost:7777/baby/Name/lat/123.123/long/123.123
            </h2>
            </p>
    </body>
    </html>
